...
# города?») — лиды, пришедшие по нашей воронке, всегда представляются
# через «Я из <город>». Любые другие первые сообщения (даже с упоминанием
# «инсты» или «бота») игнорируются — их подхватит владелец вручную,
# чтобы бот не отвечал на случайные обращения вне скрипта вроде «в бот
# хосте увидел, могу чем-то помочь?».
_LEAD_INTRO_RE = re.compile(
    r"(?<![а-яa-z])я\s+из\s+[а-яa-z]",
    re.IGNORECASE,
)


def has_lead_intro(text: str | None) -> bool:
    """True, если в тексте есть кодовая фраза «Я из …»."""
    if not text:
        return False
    norm = text.replace("ё", "е").replace("Ё", "Е")
    return bool(_LEAD_INTRO_RE.search(norm))
